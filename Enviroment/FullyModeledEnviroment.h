...
public:
    FullyModeledEnviroment(stateType* stateSpace, int stateCount,
                           ...
                           vector<pair<double, pair<const stateType&, int>>>* (*Model)(const actionType&, 
                                                                                       const stateType&));
    pair<const stateType&, int> Interact(const actionType& action);
    const stateType& InitiateEnviroment();
    vector<pair<double, pair<const stateType&, int>>>* GetAllProbabilities(const actionType& action, 
